...
from db_main import *  # imports all the database from the db_main script !
import time
try:
...
class FilestoTextFiles:
    ...
    def __init__(self):
        ...
        # db = DB()  # Creates the database object !
        self.db.delete_ALL()  # This emptifies the intial tables so that the data is later processes
        self.db.create_tables() # Creates the tale from scratch

    # The below function writes to a file_name as .txt extension, for later onward.
    def save_to_txt(self, text_data, file_name,pdflocation="",doclocation=""):
        print(file_name)
        # if str(file_name).__contains__('.pdf'):
        #     pdf_check = True
        # else:
        #     pdf_check = False  # This is just for differentiation between the pdf and the word documents .
        ...
        #if pdf_check:
        if pdflocation != "":
            pdf_id = self.db.select_id_from_pdf(pdflocation)
            self.db.insert_into_file_(FILENAME=exact_path, FILE_LOCATION=file_name, PDF_ID=pdf_id)  # INserts into the database
        else:
            doc_id = self.db.select_id_from_doc(doclocation)
            self.db.insert_into_file_(FILENAME=exact_path, FILE_LOCATION=file_name, DOC_ID=doc_id) #inserts into the doc id
        # print(exact_path)
        # print(file_name)
        # time.sleep(10) # Just for debugging
    ...
    def convert_pdf(self, pdf_filelocation):
        if pdf_filelocation != "":
            ...
            # -- INSERTING TO THE DATABASE OF THE PDF FILE -- #
            self.db.insert_into_pdf(pdf_filelocation)  # This inserts in to pdf file location ,
    ...
    def convert_list_pdffiles(self, list_pdfs):  # Converts the lost pf the pdfs
        ...
        if len(list_pdfs) >= 1 and type(list_pdfs) == list:
            for filelocation in list_pdfs:
                ...
                # self.db.insert_into_file_(FILENAME=file_name,FILE_LOCATION=)
                print("Converted -->" + file_name)  # Prints the file name
                self.save_to_txt(text_converted_file, file_name,pdflocation = filelocation)  # --Saving to .txt file
    ...
    def get_AllwordFiles(self):
        ...
        for filename in os.listdir(self.dir_path + "/Files"):  # listing the current directory files
            if filename.endswith('.word') or filename.endswith('.docx') or filename.endswith('.doc'):
                ...
                # Inserting into database of the docs
                self.db.insert_into_doc(self.dir_path + "/Files/" + filename)
                # ------------- This is the insertion of the database
                wordFiles.append(self.dir_path + "/Files/" + filename)  # adds the file name in the word list
    ...
    def convert_allwordFiles(self, wordfilelist):
        if type(wordfilelist) == list and len(wordfilelist) != 0:
            for single_wordFile in wordfilelist:
                ...
                self.save_to_txt(total_data, file_name,doclocation=single_wordFile)  # Passing the data and the single file name to the list
        else:
            pass
...
# The below is the test case which have been checked to obtain the PDF form the web as well as from local directory

# def testmain():
#     FTF = FilestoTextFiles()  # Creates the object of the specified file formaat
#     dir_path = os.path.dirname(os.path.realpath(__file__))
#     text_converted = FTF.testpdf()  # Converting the test pdf file
#     print(text_converted)
#     print("-" * 40)  # SEPERATING
#     file_location = dir_path + "\\My_oldcv.pdf"
#     text_converted = FTF.convert_pdf(file_location)
#     print(text_converted)
...
def test():
    dir_path = os.path.dirname(os.path.realpath(__file__))
    # print(os.listdir(dir_path +"\\Files"))  # This gets the file in the Folder of files
    # dir_path = os.path.dirname(os.path.realpath(__file__))
    # file_check = open(dir_path +'\\My_oldcv.pdf', 'rb')
    # print("done")
    # print("")
    # print(dir_path)
    # FTF = FilestoTextFiles()  # Creating the Class object
    # data = FTF.convert_pdf(dir_path +"\\Files\\"+"ICTO_ENG - Copy.pdf")
    # pdf_files = FTF.get_AllpdfFiles()
    # word_files = FTF.get_AllwordFiles()
    # print(pdf_files)
    # print(word_files)
    ...
    # #     print(file_name)
    # FTF.convert_all()  #
    FTF = FilestoTextFiles()
    lists = FTF.get_AllwordFiles()
    FTF.convert_allwordFiles(lists)


def test_all():
    FTF = FilestoTextFiles()  # Creating the Class object
    FTF.convert_all()  # Converts all the files from pdf, docx to txt . and save as txt
